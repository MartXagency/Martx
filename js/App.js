function App() {
  let e = document.querySelector(".Title"),
    t = document.querySelector(".CodeBy"),
  ...
  const c = new THREE.Scene,
    u = new THREE.WebGLRenderer({ canvas: r, antialias: !0, autoSize: !0 });
  u.setPixelRatio(window.devicePixelRatio), u.setSize(d, p);

  let w = d / p,
    l = new THREE.PerspectiveCamera(50, w, .1, 1e3);
  l.position.setZ(31),
  ...
  const E = new THREE.PointLight(16772574, .7),
    h = new THREE.AmbientLight(16777215, .6);
  c.add(h);
  ...
  // ✅ Donut color changed here to purple
  const m = new THREE.Mesh(
    new THREE.TorusGeometry(7, 2.2, 31, 100),
    new THREE.MeshStandardMaterial({
      color: new THREE.Color("#bf5af2"), // Purple
      bumpMap: y,
      roughnessMap: y,
      bumpScale: .007
    })
  ...
  const f = new THREE.Mesh(
    new THREE.TetrahedronGeometry(3.1, 0),
    new THREE.MeshStandardMaterial({
      color: i,
      bumpMap: y,
      roughnessMap: y,
  ...
  S.add(m),
    ...
    Array(400).fill().forEach(function () {
      const e = new THREE.Mesh(
        new THREE.SphereGeometry(.22, 13, 13),
        new THREE.MeshPhysicalMaterial({ color: s, roughness: .13 })
      ),
        [t, o, a] = Array(3).fill().map(() => THREE.MathUtils.randFloatSpread(220));
      e.position.set(t, o, a),
        e.castShadow = !0,
        e.receiveShadow = !0;
  ...
  u.shadowMap.enabled = !0,
    ...
    gsap.timeline({
      defaults: {
        duration: 1.3,
        ease: "power3.inOut",
        stagger: { from: "end", amount: .7 }
      }
    })
      .from(e, { xPercent: -31, opacity: 0 }, "<+=0.00")
      .from(t.querySelector("span"), { yPercent: 31 }, "<+=0.00")
      .to(t.querySelector("span"), { opacity: 1 }, "<+=0.00")
    ...
    gsap.timeline({
      ...
      scrollTrigger: {
        ...
        onUpdate: function (e) {
          e.progress > .031
            ? gsap.to(o.querySelector("span"), { opacity: 0, duration: 1.3, ease: "power3.out" })
            : gsap.to(o.querySelector("span"), { opacity: 1, duration: 1.3, ease: "power3.out" })
        }
      }
    })
      .to(e, { y: -700 }, "<+=0.00")
      ...
      .to(l.rotation, { y: .13 * Math.PI, z: .7 * Math.PI }, "<+=0.00")
  ...
  T.add(l),
    ...
    window.addEventListener("mousemove", function (e) {
      ...
      function n(e, t, o, a, n) {
        return XÆAXii = t - o,
          iXÆAXi = a - n,
          Number(((e - o) * iXÆAXi / XÆAXii + n).toFixed(4))
      }

      a < 0 && (a *= -1),
        iiXÆAX = n(t / 2 - o, t / 2, 0, 1, 0),
        XiiXÆA = n(a, t / 2, 0, 1, 0),
...
window.onload = () => {
  App(),
    gsap.to("body", { opacity: 1, duration: .5 })
};
